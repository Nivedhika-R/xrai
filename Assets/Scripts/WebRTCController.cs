...
public class WebRTCController : Singleton<WebRTCController>
{
    ...
    private void Start()
    {
        XAIRClient.Instance.OnWebRTCConnectionChanged += WebRtcConnectionChange;
    ...
    private void OnLoggedOnServer(bool connectionSuccess)
    {
        if (!connectionSuccess)
        {
            ...
            Debug.Log("Error connecting with the server. Check if the Python server is running locally.");
            SetConnectionState(WebRTCConnectionState.NotConnected);
        }
        else
        {
    ...
    {
        List<string> iceServerList = new List<string> {
