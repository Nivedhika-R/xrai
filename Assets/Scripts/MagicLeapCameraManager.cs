...
public class MagicLeapCameraManager : MonoBehaviour, ICameraDeviceManager
{
    ...
    private MLCamera.OutputFormat _outputFormat = MLCamera.OutputFormat.RGBA_8888; // Output format for the captured video
    private MLCamera.ConnectFlag _connectFlags = MLCamera.ConnectFlag.CamOnly; // Flags to specify the connection type and capabilities
    #endregion
    ...
    private async Task<bool> WaitForCameraAvailabilityAsync()
    {
        ...
        while (attempts < maxAttempts)
        {
            ...
            MLResult result = MLCamera.GetDeviceAvailabilityStatus(_cameraIdentifier, out cameraDeviceAvailable);
    ...
    private MLCamera.CaptureConfig CreateCaptureConfig(MLCamera.StreamCapability streamCapability)
    {
        var captureConfig = new MLCamera.CaptureConfig
        {
            ...
            {
                MLCamera.CaptureStreamConfig.Create(streamCapability, _outputFormat)
            }
        };
    ...
    private bool GetStreamCapabilityWBestFit(out MLCamera.StreamCapability streamCapability)
    {
        ...
        var streamCapabilities = MLCamera.GetImageStreamCapabilitiesForCamera(_captureCamera, MLCamera.CaptureType.Video);
        ...
        if (MLCamera.TryGetBestFitStreamCapabilityFromCollection(streamCapabilities, _targetImageWidth, _targetImageHeight, MLCamera.CaptureType.Video, out streamCapability))
        {
    ...
    /// </summary>
    private void OnCaptureRawVideoFrameAvailable(MLCamera.CameraOutput frameInfo, MLCamera.ResultExtras resultExtras, MLCamera.Metadata metadataHandle)
    {
        if (!_updatingTexture)
        {
